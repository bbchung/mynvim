		...
		config = function()
			...
			local function action_start_line(action, uri)
				...
				for _, change in ipairs(edit.documentChanges or {}) do
					...
					if
						change.textDocument
						and change.textDocument.uri == uri
						and text_edits
						and text_edits[1]
						and text_edits[1].range
					then
			...
			local function resolve_actions(bufnr, actions, callback)
				...
				for index, item in ipairs(actions) do
					...
					if
						...
						or not item.client:supports_method("codeAction/resolve")
					then
				...
				for _, response in pairs(results) do
					...
					if client then
						for _, action in ipairs(response.result or {}) do
							if
								not action.disabled
								and (action.edit or action.command or client:supports_method("codeAction/resolve"))
							then
			...
			vim.api.nvim_create_user_command(
				"LspFixAll",
				FixAll,
				{ desc = "Apply all available LSP fixes in the current buffer" }
			)
